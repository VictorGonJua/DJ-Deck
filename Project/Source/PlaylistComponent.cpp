...
{    
    // adding columns to playlist
    tableComponent.getHeader().addColumn("Track title", 1, 200);
    tableComponent.getHeader().addColumn("Duration", 2, 200);
    tableComponent.getHeader().addColumn("Deck1", 3, 125);
    tableComponent.getHeader().addColumn("Deck2", 4, 125);
    tableComponent.getHeader().addColumn("Del.", 5, 150);
    // adding and making visible the playlist table, load button and library search (as well as respective listeners)
    ...
    //creates a text file named playlist.txt and inserts a placeholder "0" as to which deckGUI to be put into (1 or 2)
    std::ofstream myfile("playlist.txt");
    myfile << stdstring << std::endl << "0" << std::endl;
    myfile.close();

    //creates a text file named PlaylistHistory.txt, which stores the current playlist audio file URLs so that the playlist data persists when reopening
...
void PlaylistComponent::resized()
{
    ...
    libraryLoadButton.setBounds(0, 0, getWidth() / 5 * 2, rowH/2);
    librarySearch.setBounds(getWidth() / 5 * 2, 0, getWidth() / 5 * 3, rowH/2);
    tableComponent.setBounds(0, rowH/2, getWidth(), getHeight());

    //playlist header columns
    tableComponent.getHeader().setColumnWidth(1, (getWidth() / 8) * 2.7);
    tableComponent.getHeader().setColumnWidth(2, (getWidth() / 8) * 1.9);
    tableComponent.getHeader().setColumnWidth(3, (getWidth() / 8) * 1.3);
    tableComponent.getHeader().setColumnWidth(4, (getWidth() / 8) * 1.3);
    tableComponent.getHeader().setColumnWidth(5, (getWidth() / 8) * 1);

    tableComponent.setColour(juce::ListBox::backgroundColourId, juce::Colours::black);
    
    //search bar
    librarySearch.setTextToShowWhenEmpty("Search for tracks...", juce::Colours::indianred);
    librarySearch.setFont(14.0f);
...
juce::Component* PlaylistComponent::refreshComponentForCell(int rowNumber,
    ...
    if (columnId == 3) {//column 3 is a button that loads the audio file into deck 1
        if (existingComponentToUpdate == nullptr) {
            juce::TextButton* btn = new juce::TextButton{ "Load" }; //creates button with deck 1 as the text
    ...
    if (columnId == 4) {//column 4 is a button that loads the audio file into deck 2
        if (existingComponentToUpdate == nullptr) {
            juce::TextButton* btn = new juce::TextButton{ "Load" }; //creates button with deck 2 as the text
            juce::String id{ std::to_string(rowNumber) };
    ...
    if (columnId == 5) {//column 5 is a button that deletes the audio file
        if (existingComponentToUpdate == nullptr) {
            juce::TextButton* btn = new juce::TextButton{ "Del." }; //creates button with delete as the text
            juce::String id{ std::to_string(rowNumber) };
            btn->setComponentID(id);
            btn->addListener(this);
            existingComponentToUpdate = btn;
